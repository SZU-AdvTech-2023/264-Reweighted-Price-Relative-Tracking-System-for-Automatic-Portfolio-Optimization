...
class GPOLU(Algo):
    ...
    def keep_top_five(self,series_data):
        # 获取前五大的元素的索引
        top_five_indices = series_data.argsort().nlargest(5).index

        # 将除了前五大元素以外的元素都设为0
        series_data.loc[~series_data.index.isin(top_five_indices)] = 0
    ...
    def OLU(self, pt, xt, eta, alpha, beta, max_iter=100, tol=1e-4):
        ...
        def minifun(p):
            -eta*(np.log(np.dot(p.T,xt))+np.dot(xt.T,p-pt)/np.dot(p.T,xt))+0.5*np.linalg.norm(p-pt, ord=2)**2
...
if __name__ == "__main__":
    tools.quickrun(GPOLU(eta=26, beta=0.1, gama=0.001))
