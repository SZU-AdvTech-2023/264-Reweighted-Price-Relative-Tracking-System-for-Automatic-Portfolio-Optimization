...
class ListResult(list, PickleMixin):
    ...
    def plot(
        ...
        **kwargs,
        ...
        # print("EG算法:")
        # print(EG_algo.summary())
        # d["EG"] = EG_algo.equity
        # d[["EG"]].plot(**kwargs)
        #
        # from .algos import OLU
        #
        # OLU_algo = OLU().run(self[0].X.cumprod())
        # OLU_algo.fee = self[0].fee
        # print("olu算法:")
        # print(OLU_algo.summary())
        # d["OLU"] = OLU_algo.equity
        # d[["OLU"]].plot(**kwargs)

        from .algos import RPRT

        RPRT_algo = RPRT().run(self[0].X.cumprod())
        RPRT_algo.fee = self[0].fee
        print("RPRT算法:")
        print(RPRT_algo.summary())
        d["RPRT"] = RPRT_algo.equity
        d[["RPRT"]].plot(**kwargs)
